...
pub const Player = struct {
    ...
    pub fn updatePlayer(p: *Player, bullets : []Bullet ) void
    {
    ...
    fn updatePlayerKeyboard(p :*Player, bullets: []Bullet ) void
    {
        ...
        if (p.is_thrusting) {
            ...
            const cosA = @cos(p.rotation * rad_per_deg);
            const sinA = @sin(p.rotation * rad_per_deg);
    ...
    fn updatePlayerMouse(p :*Player, bullets: []Bullet )
    {
        // Get mouse position
        ...
        var angle_diff = target_angle - player->rotation;
        ...
        player->rotation_velocity = angle_diff * 0.1;
        ...
        if (p.is_thrusting) {
            ...
            const current_speed = @sqrt(p.velocity.x * p.velocity.x + p.velocity.y * p.velocity.y);
            if (current_speed > 5.0) {
                p.velocity.x = (player->velocity.x / current_speed) * 5.0;
                p.velocity.y = (player->velocity.y / current_speed) * 5.0;
            }
        }
