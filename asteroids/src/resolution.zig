...
pub const Resolution = struct { // NEW added structure for resolutions
    width: i32,
    height: i32,
    name: [:0]const u8,
    ...
    pub fn toggleFullscreenMode(g: *Game) void {
        ...
        if (!rl.isWindowFullscreen()) {
            ...
            g.default_screen_width = g.current_screen_width;
            g.default_screen_height = g.current_screen_height;
            ...
            g.current_screen_width = monitor_width;
            g.current_screen_height = monitor_height;
            ...
            g.current_screen_width = cur_res.width;
            g.current_screen_height = cur_res.height;
    ...
    fn handleResolutionChange(g: *Game) void {
        ...
        g.player.position.x = @floatFromInt(@divTrunc(g.current_screen_width, 2));
        g.player.position.y = @floatFromInt(@divTrunc(g.current_screen_height, 2));
    }

pub fn changeResolution(g :*Game, new_resolution_index:i32) void
{
    if (new_resolution_index < 0 || new_resolution_index >= max_resolutions) {
    ...
    rl.setWindowPosition(
        (display_width -  new_res.width) / 2, 
        (display_height - new_res.height) / 2
    );

    // Update screen dimension globals
    g.current_screen_width =  new_res.width;
    g.current_screen_height = new_res.height;

    // Update current resolution index
    g.current_resolution = new_resolution_index;
